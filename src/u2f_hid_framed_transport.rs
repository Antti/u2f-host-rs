...
pub trait U2FHidFramedTransport {
    ...
    fn read_response(&mut self, expected_cmd: u8) -> Result<Vec<u8>> {
        ...
        if cmd != expected_cmd {
            return Err(ErrorKind::Protocol(format!("Unexpected response cmd. Expected 0x{:x}. Got 0x{:x}",
                                                   expected_cmd,
                                                   cmd))
                               .into());
        }
        ...
        while data.len() < total_data_len as usize {
            ...
            let (seq, mut cont_data) = match cont_frame.frame_content {
                ...
                _ => return Err(ErrorKind::Protocol("Expected cont frame. Got init".to_string()).into()),
            };
            if seq == current_sequence {
                data.append(&mut cont_data);
            } else {
                return Err(ErrorKind::Protocol(format!("Sequence error. Expected: {}. Got: {}",
                                                       current_sequence,
                                                       seq))
                                   .into());
            }
    ...
    fn send_cmd<T: AsRef<[u8]>>(&mut self, cmd: u8, channel_id: u32, data: T) -> Result<()> {
        ...
        let frame = U2FFrame {
            ...
            frame_content: U2FFrameContent::Init {
                cmd: cmd,
        ...
        while data.len() > datasent {
            debug!("datasent: {}, frame_data.len(): {}, data.len(): {}",
                   datasent,
                   frame_data.len(),
                   data.len());
            ...
            let frame = U2FFrame {
                channel_id: channel_id,
                frame_content: U2FFrameContent::Cont {
                    seq: sequence,
                    data: frame_data.to_vec(),
                },
            };
    ...
    fn read_frame(&mut self) -> Result<U2FFrame> {
        ...
        match res {
            ...
            Some(other_len) => {
                return Err(ErrorKind::Protocol(format!("HID read returned unexpected data size: {}. Expected 64",
                                                       other_len))
                                   .into())
            }
