...
impl <'a> APDU<'a> {
    ...
    pub fn send_apdu<T: AsRef<[u8]>>(&mut self, cmd: u8, p1: u8, p2: u8, cmd_data: T) -> Result<APDUResponse> {
        ...
        match sw {
            ...
            0x6984 => Err(ErrorKind::APDUError(sw).into()), // SW_WRONG_DATA
            0x6985 => Err(ErrorKind::APDUError(sw).into()), // SW_CONDITIONS_NOT_SATISFIED
            0x6d00 => Err(ErrorKind::APDUError(sw).into()), // SW_INS_NOT_SUPPORTED
            0x6e00 => Err(ErrorKind::APDUError(sw).into()), // SW_CLA_NOT_SUPPORTED
